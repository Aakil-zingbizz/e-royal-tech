...
const BuildingIcon = () => (
  <svg className="w-10 h-10 text-[#0a192f]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.25">
        ...
        <div className="flex flex-wrap justify-center gap-6 sm:gap-8 lg:gap-10 w-full">
          {industries.map((industry, index) => (
            <FadeInSection 
              ...
              animation="fade-up" 
              delay={index * 100}
              className="w-[calc(50%-12px)] sm:w-[calc(33.333%-22px)] lg:w-[calc(20%-32px)]"
            >
              <div 
                className="bg-white rounded-xl border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-md transition-shadow duration-300 p-4 sm:p-6 flex flex-col items-center justify-center text-center aspect-square h-full w-full"
              >
                <div className="w-20 h-20 rounded-2xl bg-blue-50 flex items-center justify-center mb-4 lg:mb-6">
                  {industry.icon}
                </div>
                <p className="text-sm lg:text-base font-semibold text-gray-800 leading-tight whitespace-pre-line">
                  {industry.name}
                </p>
              </div>
            </FadeInSection>
